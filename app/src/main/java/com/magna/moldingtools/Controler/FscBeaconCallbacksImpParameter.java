...
import com.magna.moldingtools.Activity.ParameterSettingActivity;
...
import static com.magna.moldingtools.Activity.ParameterSettingActivity.SUCESSFUL_COUNT;
import static com.magna.moldingtools.Activity.ParameterSettingActivity.isModule_BP671;
import static com.magna.moldingtools.Activity.SetActivity.OPEN_TEST_MODE;
...
public class FscBeaconCallbacksImpParameter extends FscBeaconCallbacksImp {
    ...
    public void connectProgressUpdate(BluetoothDevice device, int status) {
        ...
        InfoDialog dialog = parameterSettingActivityWeakReference.get().getConnectDialog();
        switch (status) {
            case CommandBean.PASSWORD_CHECK:
                dialog.setInfo("check password...");
                break;
            case CommandBean.PASSWORD_SUCCESSFULE:
                dialog.setInfo("password sucessful...");
                fscBeaconApi.startGetDeviceInfo(moduleString, fb);
                break;
            case CommandBean.PASSWORD_FAILED:
                dialog.setInfo("password failed...");
                parameterSettingActivityWeakReference.get().connectFailedHandler();
                break;
            case CommandBean.PASSWORD_TIME_OUT:
                dialog.setInfo("timeout");
                Log.e(TAG, "connectProgressUpdate: 1111111111111111111");
                parameterSettingActivityWeakReference.get().connectFailedHandler();
                break;
        }
    ...
    public void atCommandCallBack(String command, String param, String status) {
            if (parameterSettingActivityWeakReference.get() == null) { return;       }
            InfoDialog dialog = parameterSettingActivityWeakReference.get().getConnectDialog();
         ...
         */
        if (CommandBean.COMMAND_FINISH.equals(status) && "save...".equals(dialog.getInfo())) {
            /**
             *  if you want to switch activity please wait a moment for release service connection
             */
            dialog.setInfo("finish...");
        ...
        if (CommandBean.COMMAND_TIME_OUT.equals(status) && "save...".equals(dialog.getInfo())) {
            /**
             *  if you want to switch activity please wait a moment for release service connection
             */
            dialog.setInfo("timeout...");
            ...
            parameterSettingActivityWeakReference.get().getHandler().postDelayed(new Runnable() {
                ...
                public void run() {
                   dialog.dismiss();
                    if (OPEN_TEST_MODE) {
                        SetActivity.actionStart(parameterSettingActivityWeakReference.get());
                    } else {
                        MainActivity.actionStart(parameterSettingActivityWeakReference.get());
                    }
                    parameterSettingActivityWeakReference.get().finishActivity();
                    Log.i("timeout", "1");
                }
        ...
        if (CommandBean.COMMAND_FINISH.equals(status) && "connected".equals(dialog.getInfo())) {
